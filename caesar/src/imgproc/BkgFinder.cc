...
BkgData* BkgFinder::FindBkg(Img* img,int estimator,bool computeLocalBkg,int boxSizeX,int boxSizeY, double gridStepSizeX,double gridStepSizeY,bool use2ndPass,bool skipOutliers,double seedThr,double mergeThr,int minPixels)
{
	//## Check input image
	if(!img) {
		cerr<<"BkgFinder::FindBkg(): ERROR: Null ptr to given image!"<<endl;
		return 0;
	}
	...
	if(!img->HasStats()){//computing stats
		if( img->ComputeStats(true,false,false)<0 ){
			cerr<<"BkgFinder::FindBkg(): ERROR: Failed to compute stats!"<<endl;
			return 0;	
		}
	}//close if
	...
	if(ComputeGlobalBkg(bkgData,img,estimator)<0){
		cerr<<"BkgFinder::FindBkg(): ERROR: Failed to compute global bkg!"<<endl;
		delete bkgData;
		bkgData= 0;
		return 0;
	}
	...
	//## Compute local bkg?		
	if(computeLocalBkg){
		cout<<"BkgFinder::FindBkg(): INFO: Computing local bkg..."<<endl;
		int status= FindLocalGridBkg(bkgData,img,estimator,boxSizeX,boxSizeY,gridStepSizeX,gridStepSizeY,use2ndPass);
		if(status<0){
			cerr<<"BkgFinder::FindBkg(): ERROR: Failed to compute local grid bkg!"<<endl;
			delete bkgData;
			bkgData= 0;
			return 0;
		}
		cout<<"BkgFinder::FindBkg(): INFO: Local bkg computation completed!"<<endl;	
	...
	if(skipOutliers){
		cout<<"BkgFinder::FindBkg(): INFO: Skipping outliers..."<<endl;
		
		//Get significance map
		Img* significanceMap= img->GetSignificanceMap(bkgData,computeLocalBkg);
		if(!significanceMap){
			cerr<<"BkgFinder::FindBkg(): ERROR: Failed to compute the significance map (needed to exclude blobs)!"<<endl;
			delete bkgData;
			bkgData= 0;
			return 0;
		}	
		...
		if(status<0){
			cerr<<"BkgFinder::FindGridBkg(): ERROR: Failed to find significant blobs!"<<endl;
			delete bkgData;
			bkgData= 0;
		...
		if(!img_wOutliers){
			cerr<<"BkgFinder::FindGridBkg(): ERROR: Failed to cmpute image with blob outliers subtracted!"<<endl;
			delete bkgData;
			bkgData= 0;
			significanceMap->Delete();
		...
		if(!robustBkgData){
			cerr<<"BkgFinder::FindGridBkg(): ERROR: Failed to compute bkg over image with blob outliers subtracted!"<<endl;
			delete bkgData;
			bkgData= 0;
			significanceMap->Delete();
			for(unsigned int i=0;i<blobs.size();i++){
...
int BkgFinder::FindLocalGridBkg(BkgData* bkgData,Img* img,int estimator,int boxSizeX, int boxSizeY, double gridStepSizeX, double gridStepSizeY,bool use2ndPass){
	...
	if(ComputeLocalGridBkg(bkgData,img, estimator, boxSizeX, boxSizeY, gridStepSizeX,gridStepSizeY)<0){	
		cerr<<"Img::FindLocalGridBkg(): ERROR: Computation of local background failed for this image!"<<endl;
		return -1;
	}

	//## Improve rms by recomputing stuff from residual map 
	if(use2ndPass){
		cout<<"Img::FindLocalGridBkg(): INFO: Improving rms estimation with a 2nd pass..."<<endl;
		...
		if(ComputeLocalGridBkg(bkgData_residual,residualMap,estimator,boxSizeX, boxSizeY, gridStepSizeX,gridStepSizeY)<0){
			cerr<<"Img::FindLocalGridBkg(): ERROR: Computation of local background failed @ second pass!"<<endl;
			delete residualMap;
			residualMap= 0;
		...
		cout<<"Img::FindLocalGridBkg(): INFO: Update bkg RMS sampling data in image data..."<<endl;
		...
		cout<<"Img::FindLocalGridBkg(): INFO: Copying new noise map to bkg data..."<<endl;
		bkgData->CopyNoiseMap(bkgData_residual->NoiseMap);//copy new noise map (delete previous)

		//Delete stuff
		cout<<"Img::FindLocalGridBkg(): INFO: Delete allocated data..."<<endl;
		delete residualMap;
		residualMap= 0;
		delete bkgData_residual;
		bkgData_residual= 0;
...
//int BkgFinder::ComputeLocalGridBkg(BkgData* bkgData,Img* img,BkgEstimator estimator,int boxSizeX, int boxSizeY, double gridStepSizeX, double gridStepSizeY){
int BkgFinder::ComputeLocalGridBkg(BkgData* bkgData,Img* img,int estimator,int boxSizeX, int boxSizeY, double gridStepSizeX, double gridStepSizeY){
	...
	//## Check input image
	if(!img) {
		cerr<<"BkgFinder::ComputeLocalGridBkg(): ERROR: Null ptr to given image!"<<endl;
		return -1;
	}
	...
	if(boxSizeX<=0 || boxSizeX>=Nx || boxSizeY<=0 || boxSizeY>=Ny) {
		cerr<<"BkgFinder::ComputeLocalGridBkg(): ERROR: Invalid box size given (too small or larger than image size)!"<<endl;
		return -1;
	}
	if(gridStepSizeX<=0 || gridStepSizeY<=0 ){
		cerr<<"BkgFinder::ComputeLocalGridBkg(): ERROR: Invalid grid step size given (null or negative)!"<<endl;
		return -1;
	}
	...
	cout<<"BkgFinder::ComputeLocalGridBkg(): INFO: N("<<Nx<<","<<Ny<<") TileSize=("<<TileSizeX<<","<<TileSizeY<<") GridStepSize("<<gridStepSizeX<<","<<gridStepSizeY<<")"<<endl;
	...
	cout<<"BkgFinder::ComputeLocalGridBkg(): INFO: Counting number of tiles from ("<<indexX<<","<<indexY<<") up to ("<<indexX_end<<","<<indexY_end<<")..."<<endl;
	...
	cout<<"BkgFinder::ComputeLocalGridBkg(): INFO: nTilesX="<<nTilesX<<"("<<ixList.size()<<") nTilesY="<<nTilesY<<"("<<iyList.size()<<") nTiles="<<nTiles<<endl;


	//## Loop over all number of tiles and compute bkg info for each one
	cout<<"BkgFinder::ComputeLocalGridBkg(): INFO: Computing tile background..."<<endl;
...
//int BkgFinder::ComputeGlobalBkg(BkgData* bkgData,Img* img,BkgEstimator estimator){
...
//int BkgFinder::ComputeSampleBkg(BkgSampleData& bkgSampleData,Img* img,BkgEstimator estimator){
