...
ChanVeseSegmenter::CVdata* ChanVeseSegmenter::Init(Img* img,double initContourRadius){

	//## Normalize image
	double norm_min= 0;
	double norm_max= 255;
	Img* img_norm= img->GetNormalizedImage("LINEAR",norm_min,norm_max);
	if(!img_norm){
		cerr<<"ChanVeseSegmenter::Init(): ERROR: Failed to normalize input image!"<<endl;
		return 0;
	}
	...
	(pCVdata->imgMatrix)= img_norm->GetMatrix();
	if(!pCVdata->imgMatrix) {
		cerr<<"ChanVeseSegmenter::Init(): ERROR: Failed to convert input norm image to matrix!"<<endl;
		if(img_norm) img_norm->Delete();
		if(pCVdata){
			delete pCVdata;
			pCVdata= 0;
  ...
  pCVdata->edges = new TMatrixD(nRows,nCols);
	...
	double rowCenter= nRows/2.0;
	double colCenter= nCols/2.0;
	cout<<"ChanVeseSegmentation::Init(): INFO: Init contour center("<<rowCenter<<","<<colCenter<<")"<<endl;
...
Img* ChanVeseSegmenter::FindSegmentation(Img* img,bool returnContourImg,double dt,double h,double lambda1,double lambda2,double mu,double nu,double p,double initContourRadius){

	//Check input image
	if(!img){
		cerr<<"ChanVeseSegmenter::FindSegmentation(): ERROR: Null ptr to given image!"<<endl;
		return 0;
	}
  ...
  pCVinputs->p = p;
	...
	ChanVeseSegmenter::CVdata* pCVdata= Init(img,initContourRadius);
	if(!pCVdata){
		cerr<<"ChanVeseSegmenter::FindSegmentation(): ERROR: Failed to initialize!"<<endl;
		if(pCVinputs){
			delete pCVinputs;
			pCVinputs= 0;
		}
	...
	//## Run segmentation
	cout << "ChanVeseSegmenter::FindSegmentation(): INFO: Performing segmentation..."<<endl;
  CVSegmentation(pCVdata->imgMatrix,pCVdata->phi0, &(pCVdata->phi), pCVinputs);

	//## Get zero crossing pixels
  cout << "ChanVeseSegmenter::FindSegmentation(): INFO: Getting zero crossings..."<<endl;
  ZeroCrossings(pCVdata->phi,&(pCVdata->edges),norm_max,norm_min);
  *(pCVdata->imgMatrixOut)= *(pCVdata->imgMatrix);
  
	//## Fill segmented image (or contour image)
	cout<<"ChanVeseSegmenter::FindSegmentation(): INFO: Filling segmented image..."<<endl;
	TString imgName= Form("%s_CVSegmented",img->GetName());
	Img* outputImg= img->GetCloned(std::string(imgName),true,true);
	outputImg->Reset();
	...
	if(returnContourImg){
		for (int i=0;i<nRows; ++i) {
			...
			for (int j=0;j<nCols; ++j) {
				...
				double w= (pCVdata->edges)->operator()(i,j);
      	if ( w == norm_max) {
					outputImg->SetBinContent(ix+1,iy+1,1);
				}
    	}//end loop 
  	}//end loop
