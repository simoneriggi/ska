...
SLICData* SLICUtils::SPGenerator(Image* img, int regionSize,double regParam, int minRegionSize, bool normalizeImage, bool useLogScaleMapping, Image* laplImg, Image* edgeImg){
...
int SLICUtils::FindNeighbors(std::vector<SLICNeighborCollection>& neighbors,SLICData* slicData,SLICContourData* contourData,bool get2ndNeighbors,int selectedTag,bool includeSpatialDist,bool normalizeParams,bool useRobustParams,bool addCurvDist){
	...
	for(int i=0; i<nRegions; i++) {
		...
		for(unsigned int j=0;j<(contourData->connectedRegionIds)[i].size();j++){//loop over 1st neighbors
			...
			if(selectedTag==-1 || neighborTag==selectedTag){
				...
				if(status<0){
					WARN_LOG("Symm distance calculation failed!");
				}
				...
				if(status<0){
					WARN_LOG("Asymm distance calculation failed!");
				}
				//==========================
				*/
...
int SLICUtils::ComputeRegionDistance(double& Dsym,double& Dsym_space,Region* region_i,Region* region_j, RegionDistNormData normPars, bool normalizeParams,bool useRobustParams,bool addCurvDist)
{
	//Init
	Dsym= 0;
	Dsym_space= 0;	
...
int SLICUtils::ComputeRegionAsymmDistance(double& Diss,double& DissNeighbor,Region* region_i,Region* region_j, RegionDistNormData normPars, bool normalizeParams,bool useRobustParams,bool addCurvDist,bool addSpatialDist)
{
	//Init
	Diss= 0;
	DissNeighbor= 0;	
...
SLICSimilarityData* SLICUtils::ComputeRegionSimilarity(SLICData* slicData,std::vector<SLICNeighborCollection>& neighbors,double beta){
	...
	if(!edgeImgStats){
		ERROR_LOG("No stats computed for edge image!");
		return nullptr;
	}
