...
int Serializer::EncodeTaskDataToProtobuf(SourcePB::TaskData& taskData_pb,TaskData* taskData){
	...
	try {
		...
		for(unsigned int i=0;i<(taskData->sources).size();i++){
			SourcePB::Source* thisSourcePB = taskData_pb.add_sources();
			if(EncodeSourceToProtobuf(*thisSourcePB,(taskData->sources)[i])<0){
				std::stringstream errMsg;
		...
		for(unsigned int i=0;i<(taskData->ext_sources).size();i++){
			SourcePB::Source* thisSourcePB = taskData_pb.add_ext_sources();
			if(EncodeSourceToProtobuf(*thisSourcePB,(taskData->ext_sources)[i])<0){
				std::stringstream errMsg;
...
int Serializer::EncodeTaskDataCollectionToProtobuf(SourcePB::TaskDataCollection& taskDataCollection_pb,std::vector<TaskData*> taskDataCollection){

	//Fill task collections
	for(unsigned int i=0;i<taskDataCollection.size();i++){
		SourcePB::TaskData* thisTaskPB = taskDataCollection_pb.add_tasks();
...
int Serializer::TaskDataCollectionToBuffer(SBuffer& buffer,std::vector<TaskData*> taskDataCollection){
	...
	try {
		//## Create google protobuf source message
		SourcePB::TaskDataCollection taskDataCollection_pb;
...
int Serializer::EncodeProtobufToTaskData(TaskData& taskData,const SourcePB::TaskData& taskData_pb){
	...
	try {		
		...
		for(int i=0;i<taskData_pb.sources_size();i++){		
			const SourcePB::Source& thisSourcePB= taskData_pb.sources(i);
			aSource= new Source;
		...
		for(int i=0;i<taskData_pb.ext_sources_size();i++){		
			const SourcePB::Source& thisSourcePB= taskData_pb.ext_sources(i);
			aExtSource= new Source;
			if(EncodeProtobufToSource(*aExtSource,thisSourcePB)<0){
				delete aExtSource;
				aExtSource= 0;
				throw std::runtime_error("Failed to encode source in collection from protobuf!");	
			}
...
int Serializer::EncodeProtobufToTaskDataCollection(std::vector<TaskData*>& taskDataCollection,const SourcePB::TaskDataCollection& taskDataCollection_pb,bool isTaskCollectionPreAllocated){
	...
	if(!isTaskCollectionPreAllocated){
		try {		
			...
			for(int i=0;i<taskDataCollection_pb.tasks_size();i++){		
				const SourcePB::TaskData& thisTaskPB= taskDataCollection_pb.tasks(i);
