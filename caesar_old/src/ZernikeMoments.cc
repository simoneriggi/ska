...
/*
std::vector<double> ZernikeMoments::RadialPolynomials(){
...
std::vector<double> ZernikeMoments::GetZernike2D_Direct(Img* img, double order, double radius) {
	...
	cout<<"ZernikeMoments::GetZernike2D_Direct(): INFO: (Nx,Ny)=("<<Nx<<","<<Ny<<") R="<<R<<endl;
	...
	for (int i=0;i<Nx;i++) {
		...
		for (int j=0;j<Ny;j++) {
			...
			*/
			...
			double x= img->GetXaxis()->GetBinCenter(i+1) - m10_m00;
			double y= img->GetYaxis()->GetBinCenter(j+1) - m01_m00;
			...
			//cout<<"(Nx,Ny)="<<Nx<<","<<Ny<<") M("<<m10_m00<<","<<m01_m00<<") (ix,iy)=("<<ix<<","<<iy<<"), (x,y)=("<<x<<","<<y<<") (x0,y0)="<<x0<<","<<y0<<") r="<<r<<", r0="<<r0<<" theta="<<theta<<" theta0="<<theta0<<" p="<<p<<" R="<<R<<endl;
			...
			int theLUT = 0;
			for (unsigned int theZ=0;theZ<zsum.size();theZ++) {
				int n = n_s[theZ];
				int l = l_s[theZ];
	...
	npixInCircle+= 1;
	cout<<"ZernikeMoments::GetZernike2D_Direct(): INFO: npixInCircle="<<npixInCircle<<" wSumInCircle="<<wSumInCircle<<endl;
	...
	for (unsigned int theZ=0;theZ<zsum.size();theZ++) {
		...
		cout<<"(n,m)=("<<n<<","<<m<<"), Z="<<Re<<"+"<<Im<<"i, A="<<ampl<<" phase="<<phase<<endl;
	}

	return moments;

}//close ZernikeMoments::GetZernike2D_Direct()
...
std::vector<double> ZernikeMoments::mb_Znl(double *X, double *Y, double *P, int size, double D, double m10_m00, double m01_m00, double R, double psum) {
	...
	for (theZ = 0; theZ < numZ; theZ++) {
		...
		cout<<"(n,m)=("<<n<<","<<m<<"), Z="<<preal<<"+"<<pimag<<"i, A="<<ampl<<" phase="<<phase<<endl;
	}
...
std::vector<double> ZernikeMoments::GetZernike2DOld(Img* img, double D, double R) {
	
	double *Y,*X,*P,psum;
	double intensity;
	int x,y,size;

	int rows = img->GetNbinsY();
	int cols = img->GetNbinsX();
	if (D<=0) D=15;
	if (R<=0) R=rows/2;
	...
	for (y=0;y<rows;y++){
		double binY= img->GetYaxis()->GetBinCenter(y+1);
		for (x=0;x<cols;x++) {
			double binX= img->GetXaxis()->GetBinCenter(x+1);

			intensity = img->GetBinContent(x+1,y+1);
			if (intensity != 0) {
				Y[size] = binY;
				X[size] = binX;
				P[size] = intensity;
...
//void ZernikeMoments::mb_zernike2D (Img* img, double order, double rad, double* zvalues, long* output_size) {
std::vector<double> ZernikeMoments::GetZernike2D (Img* image, double order, double rad) {
  ...
  //output_size -* long- the number of enteries in the array "zvalues" (normally 72)
	...
	Img* img= image->GetNormalizedImage(0,1);
	...
	for (int i=0;i<Nx;i++) {
		...
		x= (img->GetXaxis()->GetBinCenter(i+1)-m10_m00)/Rmax;

		for (int j=0;j<Ny;j++) {
			// In the paper, the center of the unit circle was the center of the image
			//y = (double)(2*j+1-N)/(double)D;
			//y= (img->GetYaxis()->GetBinCenter(j+1)-m01_m00)/(double)D;
			y= (img->GetYaxis()->GetBinCenter(j+1)-m01_m00)/Rmax;
